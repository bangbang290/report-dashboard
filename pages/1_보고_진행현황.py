...
def render_report_card(r, user, key_prefix=""):
    ...
    with st.container(border=True):
        st.markdown(
            f"**{r['team_name'] or '(이름 미입력)'}**  \n"
            f"{r['status']} · {format_schedule_display(r['scheduled_date'], r['scheduled_time'])}"
        )
        ...
        if can_edit:
            ...
            if btn_col1.button("수정", key=f"edit_btn_{kp}", use_container_width=True):
                st.session_state[edit_key] = not st.session_state.get(edit_key, False)
                st.session_state[confirm_key] = False

            if btn_col2.button("🗑️ 삭제", key=f"delete_btn_{kp}", use_container_width=True):
                ...
                if save:
                    ...
                    if not new_team_detail.strip():
                        ...
                        ok, conflicts, suggested = db.edit_report(
                            ...
                            new_date.isoformat(), new_time, r["memo"] or "",
